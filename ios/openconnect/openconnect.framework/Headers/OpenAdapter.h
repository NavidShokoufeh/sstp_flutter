...
typedef void(^AdapterCompletion)(int status, NSString *_Nullable msg);
...
configureTunnelWithNetworkSettings:(NEPacketTunnelNetworkSettings *_Nullable)networkSettings
     completionHandler:(void (^_Nullable)(NEPacketTunnelFlow *_Nullable packetFlow))completionHandler
 ...
 */
@property (nonatomic, weak, nullable) id<OpenAdapterDelegate> delegate;
...
/**
 Starts the tunnel.
 */
- (void)connect:(NSString *_Nullable) host user:(NSString *_Nullable)username pass:(NSString *_Nullable)password udid:(NSString *_Nullable) udid completion:(AdapterCompletion _Nullable)completion;
- (void)disconnect;
@end
